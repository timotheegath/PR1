...
The performance of the different fusion rules was tested and plotted in Figure \ref{fig:conf_matrix_ensemble} and \ref{fig:acc_unit_error}, with parameter randomisation and bagging enabled. In this setup, the majority voting rule performed better than both product and mean rules, which both performed equally well. Because of the randomness introduced to the units, it is understandable how the product and mean fusion rules performed less well (more susceptible to noise).  In Figure  
\begin{figure}
    \centering
