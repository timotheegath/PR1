...
In Figure \ref{fig:} we can see results of decreasing $M_{pca}$ while retaing all the fisherfaces ($c-1$).
Our results show no particular trend in decreased classification accuracy. This supports the fact that we can learn much better features when applying supervised training with knowledge of classes, making the model more successful in sparse feature space.

\subsection{Ranks of the scatter matrices}
The scatter matrices are defined as sums of outer vector products. We confirm that the rank of the within-class-scatter matrix (Sw) is N-c. As the matrix obtained by vector outer product can have at most rank 1, and these outer products are summed for each class, the rank of the summed matrix would be Ni, where Ni is the number of vectors per class. 
